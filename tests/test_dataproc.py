...
class CoordTests(unittest.TestCase):
    ...
    def test_get_ecef_y(self):
        ecef_on_y_axis = GPSCoord(0.0, np.pi / 2, 0.0).get_ecef()
    ...
    def test_get_ecef_z(self):
        ecef_on_z_axis = GPSCoord(np.pi / 2, 0.0, 0.0).get_ecef()
    ...
    def test_get_ecef_mixed(self):
        ecef_mixed = GPSCoord(np.pi / 4, np.pi / 4, 0.0).get_ecef()
    ...
    def test_to_local_for_straight_north(self):
        north_coord = GPSCoord(np.pi / 4, 0.0, 0.0)
    ...
    def test_to_local_for_straight_east(self):
        east_coord = GPSCoord(0.0, np.pi / 2, 0.0)
    ...
    def test_to_local_for_mixed(self):
        mixed_coord = GPSCoord(np.pi / 4, np.pi / 4, 0.0)
