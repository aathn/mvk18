  ...
  def run(self):
    if not os.path.isfile('./skysensestreamer/maestro.py'):
      try:
        ...
        urllib.request.urlretrieve(maestro_url, filename='./skysensestreamer/maestro.py')
        print('Successfully downloaded maestro')
      except:
        print('Failed to download maestro, manual download from "https://github.com/FRC4564/Maestro" required')
    else:
